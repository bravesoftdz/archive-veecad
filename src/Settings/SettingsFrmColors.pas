...
type
  TSetColorsForm = class(TForm)
    ...
    procedure SchemesTComboBoxChange(Sender: TObject);
...
//    FColorPreviewer.PixelsPerCell := Editor.PixelsPerCell;
    ...
    // *** COLOR SETTINGS ***
    // shows colors from editor, and color scheme name
    ...
    // load combobox with list of available color schemes
    SchemesTComboBox.Items.Assign( Styles.Names );

    // get name of last displayed color scheme
    RegIniFile := GetRegIniFile;
    try
        SchemeName := RegIniFile.ReadString( 'Color', 'Scheme', '' );
    finally
        RegIniFile.Free;
    end;

    // select last used color scheme
    SchemeIndex := SchemesTComboBox.Items.IndexOf( SchemeName );
    if SchemeIndex >= 0 then begin
        SchemesTComboBox.ItemIndex := SchemeIndex;
    end;
end;
...
begin
    ...
    // save only color scheme to registry - editor settings are saved on
    // shutdown of VeeCAD
    RegIniFile := GetRegIniFile;
    try
        RegIniFile.WriteString( 'Color', 'Scheme', SchemesTComboBox.Text );
    finally
        RegIniFile.Free;
    end;
...
procedure TSetColorsForm.SchemesTComboBoxChange(Sender: TObject);
begin
    Styles.StyleToEditor( SchemesTComboBox.ItemIndex, ColorPreviewer );
    PreviewerToPickers;
    FColorPreviewer.Paint;
end;
